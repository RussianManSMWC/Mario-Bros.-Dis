...
;output A: is zero, no interaction, otherwise the output is the direction from which the contact had occured
CODE_C446:
   LDY #$00					;
   LDX #$00					;
...
CODE_C46E:
   ...
   CLC						;
   ADC CurrentEntity_HitBoxYPos			;entity A's hit box y-displacement.
   ADC $1D					;height
...
DATA_C510:
   ...
   .word CODE_D3A8				;last init state?
...
CODE_C5A3:
   ...
   JSR CODE_C66A				;handle entities?
   JSR CODE_E783				;some kinda timers?
   ...
   JSR CODE_E21A				;handle 1-up score reward
...
CODE_C5DB:
   ...
   LDA $19					;something Mario's
   STA $0310					;

   LDA $1B					;something Luigi's
   STA $0330					;
...
CODE_C605:
   ...
   LDX $BE					;
   INC $04AC,X					;some kinda timer (for fireball spawn?)
   INC $04AC,X					;

   LDA $C6					;
   BEQ CODE_C636				;
   AND #$F0					;10-F0 - got hurt
   BNE CODE_C629				;
...
;$C6 = 01-0F
   ...
   LDA $C6					;check if escaped the respawn platform or something
   BEQ CODE_C636				;
   AND #$0C					;bits 3 and 2
...
CODE_C62C:  
   JSR CODE_DFBA				;some kinda timer...
   JMP CODE_C657				;
...
CODE_C647:   
   LDA $51					;are ya winning, son?
   BNE CODE_C62C				;if so, some kinda timer thing again. man they really want to check you winning

   JSR CODE_D6BA				;graphics related?
   JSR CODE_C785				;movement related?
   JSR CODE_CC3F				;bumping platforms related? (calculates VRAM pos thats all i know)
...
CODE_CC54:
   ...
   JSR CODE_CA7D				;i think this calculates VRAM position for entity based on it's, well, position.
   
   LDA $00

CODE_CC5F:
   LDX $05FA                
   STA CurrentEntity_VRAMPosition+1
   STA $0520,X
   
   INX                      
   LDA $01                  
   STA CurrentEntity_VRAMPosition
   STA $0520,X
...
CODE_CEC3:
   LDA #<Entity_Address+$60			;start at $360 (enemy entities)
   STA $A0					;
...
CODE_D0B4:
   ...
   LDA PlayerScore-1,X			;\that one address that I still have no idea what is
   BEQ CODE_D0E8			;/
...
CODE_D0E4:  
   LDA PlayerScore-1,X			;need to find out what this is
   BEQ CODE_D0C1			;
...
;some other calculation routine...
CODE_D15A:
   JSR CODE_D17C			;extract digits
   SBC $01				;
   STA $01				;minus itself?
   BCS CODE_D16D
   ADC #$0A                 
...
CODE_DC92:
   LDA $C6					;if the player is hurt, dead or whatever
   BNE CODE_DC91				;don't run the consequence

   LDY #$12					;entity's something
   LDA ($14),Y					;
   BNE CODE_DC91				;no consequence

   LDY #$19					;VRAM position?
   LDA ($14),Y					;
   BNE CODE_DC91				;no consequence
...
CODE_DDC7:
   LDY #$10					;
   LDA ($14),Y					;probably check if has been collected and runs collected animation
   BNE CODE_DE04				;do nothing
   TYA						;
   LDY #$19					;VRAM pos (probably so it can't be bumped anymore)
   STA ($14),Y					;
...
CODE_DDE0:
   ...
   LDA $C6					;
   AND #$F0					;
   CMP #$20					;check if in water splash state
   BEQ CODE_DE2F				;
   ...
   LDA #$03					;move 3 pixels every frame
...
CODE_DE05:
   ...
   LDA CurrentEntity_OAMOffset			;remove this enemy's display
   JSR CODE_C3F5				;
...
CODE_DE3B:
   ...
   LDA #$01					;player's state (platform?)
   STA $C6					;
...
CODE_DE4E:  
   LDA DisableControlFlag			;check if in a win situation
...
CODE_DE5E:   
   LDA DATA_F6C8,Y				;re-initialize current entity
...
CODE_DE78:
   STY $C2					;can't be timer in this context, a pipe come-out direction?
   STY $B1					;
   STX CurrentEntity_XPos			;

   LDY #$04					;
   JMP CODE_DFC4				;remove sprite tiles (potentially a second time)
   
CODE_DE83:
   LDA DemoFlag					;
   BNE CODE_DEAE				;don't do something in the demo mode
...
CODE_DEEC:
   LDA $C6					;check the player's state
   LDY CurrentEntity_ID				;
   CMP #$01					;is set to respawn?
   ...
   LDA Player1TriggerGameOverFlag		;if not set to gameover
   BEQ CODE_DF07				;continue on

CODE_DEFE:
   LDA #$00					;
   STA CurrentEntity_ActiveFlag			;the entity is no longer active

   LDY #$06					;remove all of its sprite tiles
   JMP CODE_DFC4				;
...
;first byte is the length of string(s) in low nibble, and high nibble is number of rows (strings) to draw, by default it's always 1 string
...
DATA_E6C5:
.byte $01,$01,$03,$00,$2C,$05,$A5,$02
.byte $00,$00,$00,$00,$00,$00,$00,$F0
.byte $00,$00,$00,$00,$00,$00,$00,$00
.byte $00,$00,$00,$00,$00,$00,$0F,$0F

DATA_E6E5:
.byte $38,$24,$2C,$C8,$24,$2E,$18,$5A
.byte $2D,$2C,$5A,$30,$D4,$5A,$30,$E8
.byte $5A,$2D,$60,$8A,$2F,$A0,$8A,$2E
.byte $28,$BA,$2D,$D8,$BA,$30
...
DATA_ECCC:
.byte $01,$00,$02,$00,$33,$05,$8C,$23
.byte $00,$00,$00,$00,$4C,$F6,$00,$80
.byte $00,$00,$01,$0A,$02,$01,$00,$00
...
DATA_EF67:
.byte $77,$8A,$03,$70,$77,$8B,$03,$78
.byte $77,$8B,$43,$80,$77,$8A,$43,$88
.byte $A7,$8A,$03,$20,$A7,$8B,$03,$28
.byte $A7,$8B,$43,$30,$A7,$8A,$43,$38
.byte $A7,$8A,$03,$C0,$A7,$8B,$03,$C8
.byte $A7,$8B,$43,$D0,$A7,$8A,$43,$D8
...
DATA_EF9D:
.byte $9C,$00,$9D,$00,$9F,$00,$00 
.byte $9D,$00,$9B,$00,$00,$92,$00,$00
.byte $93,$00,$00,$94,$00,$00,$95,$00
.byte $92,$00,$93,$00,$94,$00,$FF

;initial values for reflecting fireball
DATA_EFBB:
.byte $01,$01,$03,$01,$3D,$07,$92,$02
.byte $F4,$00,$04,$00,$00,$00,$00,$B0
.byte $00,$01,$00,$00,$00,$01,$00,$00
.byte $00,$00,$00,$00,$00,$00,$04,$04

DATA_EFDB:
.byte $9C,$00,$9D,$00,$9F,$00,$9C,$00
.byte $9D,$00,$9F,$00,$9C,$00,$9D,$00
.byte $9F,$00,$9C,$00,$9D,$00,$9F,$00
.byte $9C,$00,$9D,$00,$9F,$00,$9C,$00
.byte $9D,$00,$9F,$00,$9C,$00,$9D,$00
.byte $9B,$00,$FF

DATA_F006:
.byte $9B,$00,$9C,$00,$9D,$00,$00,$9E
.byte $00,$9F,$00,$9C,$00,$9D,$00,$00
.byte $9E,$00,$9F,$00,$FF
...
DATA_F061:
.byte $01,$01,$00,$01,$3D,$07,$92,$01
.byte $F4,$00,$00,$00,$00,$00,$00,$A0
...
;.byte $0D
;.byte $16,$0A,$0D,$0E,$24,$12
;.byte $17,$24,$13,$0A,$19,$0A,$17
...
DATA_F2EC:
;Mario
.byte $01,$00,$03,$00,$00,$00,$12,$40
.byte $D0,$44,$10,$00,$00,$00,$00,$01
.byte $00,$00,$00,$00,$00,$01,$00,$00
.byte $00,$00,$00,$00,$00,$00,$04,$04

;Luigi
.byte $01,$00,$03,$00,$00,$00,$12,$01
.byte $D0,$C4,$28,$00,$00,$00,$00,$02
.byte $00,$00,$00,$00,$00,$01,$00,$00
.byte $00,$00,$00,$00,$00,$00,$04,$04
...
DATA_F33A:
.byte $01,$00
.byte $01,$00,$01,$01,$00,$01,$01,$02
.byte $01,$02,$02,$02,$02,$02,$02,$03
.byte $03,$03,$03,$AA

DATA_F350:
.byte $FC,$FC,$FC,$FC
.byte $FC,$FC,$FC,$FD,$FD,$FE,$FE,$FE
.byte $FE,$FE,$FE,$FF,$FE,$FF,$FF,$FF
...
DATA_F370:
.byte $FF,$FF,$FF,$FF
.byte $FF,$00,$FF,$00,$FF,$00,$AA

DATA_F37B:
.byte $00,$01,$00,$01,$00,$01,$01,$01
.byte $02,$01,$01,$02,$03,$03,$04,$04
.byte $CC,$04,$CC,$CC,$CC,$04,$CC,$AA

DATA_F393:
.byte $01,$03,$00,$01,$02,$00,$01,$01
.byte $00,$AA,$01,$03,$00,$01,$02,$00
.byte $01,$01,$FF,$AA

.byte $01,$02,$00,$01,$01,$FF,$01,$01
.byte $FF,$01,$01,$00,$01,$01,$00,$01
.byte $01,$01,$AA
...
DATA_F452:
.byte $01,$00,$03,$00,$0A,$06,$52,$21
.byte $28,$00,$00,$00,$4C,$F6,$00,$10
.byte $00,$00,$00,$0A,$00,$01,$00,$00
.byte $00,$00,$00,$00,$00,$00,$04,$06
.byte $01,$00,$03,$00,$10,$03,$6C,$22
.byte $28,$00,$00,$04,$4C,$F6,$00,$20
.byte $00,$00,$00,$14,$00,$01,$00,$00
.byte $00,$00,$00,$00,$00,$00,$05,$06
.byte $01,$00,$03,$00,$24,$03,$7D,$20
.byte $28,$00,$00,$08,$4C,$F6,$00,$30
.byte $00,$00,$00,$00,$00,$01,$00,$00
.byte $00,$00,$00,$00,$00,$00,$05,$06
...
DATA_F560:
.byte $22,$24,$24,$24,$24			;No POW block
.byte $22,$FE,$FF,$90,$91			;hit twice
.byte $22,$FC,$FD,$8E,$8F			;hit once
.byte $22,$FA,$FB,$8C,$8D			;full POW block
...
.byte GFX_CoinCollected,Entity_Draw_8x8

DATA_F5AA:
.byte $FD,$FE,$FE,$FE,$FE,$FF,$FF,$FF
.byte $FE,$00,$FF,$00,$FE,$00,$FF,$00
.byte $00,$00,$99
...
DATA_F5CA:
.byte $58,$40,$5A,$40,$58,$40,$5A,$30
.byte $58,$30,$5A,$20,$58,$20,$5A,$20
.byte $58,$20,$5A,$18,$58,$10,$00,$5A
.byte $10,$58,$10,$5A,$08,$58,$08,$FF

DATA_F5EA:
.byte $D9,$40,$DC,$40,$D9,$40,$DC,$30
.byte $D9,$30,$DC,$20,$D9,$20,$DC,$20
.byte $D9,$18,$DC,$10,$00,$D9,$10,$DC
.byte $08,$D9,$08,$DC,$08,$FF

DATA_F608:
.byte $E0,$60,$E3,$40,$E0,$30,$E3,$20
.byte $E0,$20,$E3,$18,$E0,$18,$E3,$10
.byte $E0,$10,$00,$E3,$08,$E0,$08,$E3
.byte $08,$E0,$08,$E3,$04,$FF

DATA_F626:
.byte $E6,$E6,$E6,$E6,$01,$E6,$E6,$E6
.byte $FF,$75,$75,$75,$75,$01,$75,$75
.byte $75,$FF              
   
DATA_F638:
.byte $01,$02,$03,$FF,$02,$01,$03,$FF
.byte $00,$03,$FF
...
DATA_F6C8:
.byte $01,$00,$02,$00,$2C,$05,$A1,$22
.byte $00,$00,$00,$00,$4C,$F6,$00,$40
.byte $00,$01,$00,$0A,$02,$01,$00,$00
.byte $00,$00,$00,$00,$00,$00,$04,$03

DATA_F6E8:
.byte $FE,$FE,$FE,$FF,$DD,$47,$FF,$FF
.byte $FF,$00,$DD,$48,$FF,$FF,$00,$FF
.byte $CC,$01,$DD,$49,$00,$00,$00,$00
.byte $00,$00,$CC,$05,$DD,$4D,$00,$00
.byte $00,$00,$00,$00,$00,$00,$EE

DATA_F70F:
.byte $FE,$FE,$FE,$FF,$DD,$EB,$FF,$FF
.byte $FF,$00,$DD,$F0,$00,$00,$00,$00
.byte $00,$00,$EE
...
DATA_F7E9:
.byte $00,$03,$FF,$03,$00,$03,$00,$03
.byte $FF,$03,$01,$03,$01,$03,$00,$03
.byte $01,$03,$02,$03,$02,$03,$02,$02
.byte $02,$02,$03,$02,$03,$02,$03,$02
.byte $03,$01,$03,$01,$03,$01,$03,$01
.byte $04,$00,$04,$01,$04,$00,$04,$01
.byte $04,$01,$AA           
